...
class ChatClient:
    ...
    def send_connect_ack(self, request, dst_ip, dst_port):
        ...
        brequest = TCPAppServerAPI.gen_request
    ...
    def send_connect_wait(self, dst_ip, dst_port):
        ...
        brequest = TCPAppServerAPI.gen_request('connect_wait', {'dst_ip': dst_ip, 'dst_port': dst_port})
    ...
    def send_connect_unknown(self, request, dst_ip, dst_port):
        ...
        brequest=TCPAppServerAPI.gen_request('connect_unknown', args_dict, '')
    ...
    def send_connect_abort(self, dst_ip, dst_port):
        ...
        self.connected_client.ip=dst_ip
        self.connected_client.port=dst_port
        brequest = TCPAppServerAPI.gen_request('connect_abort', {'dst_ip': dst_ip, 'dst_port': dst_port})
    ...
    def handle_connect_req(self, request, dst_ip, dst_port):
        ...
        if self.connected_client.status:
            ...
            if 'y'==res.lower().strip('\n'):
                ...
                brequest = TCPAppServerAPI.gen_request('connect_ack', {'dst_ip': dst_ip, 'dst_port': dst_port})
                logging.debug(f"Created connect_ack packet: {brequest}")
                SocketHelper.send(self.client_sock, brequest)
                logging.debug(f"connect_ack packet sent {dst_ip}:{dst_port}!")
                print("Connect accepted!")
            else:
                brequest = TCPAppServerAPI.gen_request('connect_req_cancel', {'dst_ip': dst_ip, 'dst_port': dst_port})
    ...
    def send_msg(self, dst_ip, dst_port):
        args_dict={'dst_ip': dst_ip, 'dst_port': dst_port}
        brequest=TCPAppServerAPI.gen_request('send_msg', args_dict, '')
    ...
    def process_input(self, s):
        ...
        if self.connected_client.status:
            ...
            brequest=TCPAppServerAPI.gen_request('send_msg', args_dict, s)
            SocketHelper.send(self.client_sock, brequest)
        else:
            cmd_list = s.split(" ")
            ...
            if cmd_list[0] == 'connect':
                ...
                request = TCPAppServerAPI.gen_request('listsocks')
            ...
            try:
                ...
                for r_fd in ready_rlist:
                    if r_fd == sys.stdin:
                        ...
                        request = TCPAppServerAPI.receive_request(self.client_sock)
